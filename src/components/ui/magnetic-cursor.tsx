...
export function MagneticCursor({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      state.pos.current.x += (state.pos.target.x - state.pos.current.x) * effectiveLerp;
      state.pos.current.y += (state.pos.target.y - state.pos.current.y) * effectiveLerp;

      const deltaX = state.pos.current.x - state.pos.previous.x;
      const deltaY = state.pos.current.y - state.pos.previous.y;
    ...
    const attachMagneticListeners = (el: HTMLElement) => {
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (el.hasAttribute(hoverAttribute)) {
              attachMagneticListeners(el);
            }
            // Also check children
            el.querySelectorAll(`[${hoverAttribute}]`).forEach((child) => {
              attachMagneticListeners(child as HTMLElement);
            });
          }
        });
      });
