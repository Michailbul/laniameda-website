...
const PHASES = [
    { label: "Pilot", desc: "Validate", color: "cyan" },
    { label: "Systemize", desc: "Build", color: "cyan" },
                        ...
                        <div className="space-y-3">
                            {ACTION_ITEMS.map((item, i) => (
                                <motion.div
                                    ...
                                    className="group flex items-start gap-4 p-4 rounded-lg border border-white/5 bg-white/[0.02] hover:border-cyan-500/20 hover:bg-white/[0.04] transition-all duration-300"
                                >
                                    <div className="shrink-0 mt-0.5">
                                        <div className="w-5 h-5 rounded border border-cyan-500/40 flex items-center justify-center group-hover:bg-cyan-500/10 transition-colors">
                                            <CheckCircle2 className="w-3 h-3 text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                                        </div>
                                    </div>
                                    <div className="flex-1">
                ...
                >
                    <div className="relative w-full max-w-[280px]">
                        ...
                        <div className="relative">
                            {/* Vertical Line */}
                            <div className="absolute left-[23px] top-8 bottom-8 w-px bg-gradient-to-b from-cyan-500/50 via-cyan-500/20 to-transparent" />
                            ...
                            {PHASES.map((phase, i) => (
                                ...
                                >
                                    ...
                                    <div className={cn(
                                        ...
                                        <span className={cn(
                                            "font-mono text-xs",
                                            phase.color === "cyan" ? "text-cyan-400" : "text-white/60"
                                        )}>0{i + 1}</span>
                                    </div>
                                    ...
                                    <div>
                                        <p className={cn(
                                            "text-lg font-light",
                                            phase.color === "cyan" ? "text-white" : "text-white/70"
                                        )}>{phase.label}</p>
                                    ...
                                    {/* Arrow (except last) */}
                                    {i < PHASES.length - 1 && (
                                        <ArrowRight className="absolute left-[44px] top-[52px] w-3 h-3 text-cyan-500/30 rotate-90" />
                                    )}
                                </motion.div>
                            ))}
                        </div>
